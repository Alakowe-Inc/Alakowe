...
function Signup() {
  ...
  const strengthColor = ['bg-gray-200', 'bg-rose-400', 'bg-amber-400', 'bg-emerald-400', 'bg-emerald-500'][strength]
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="w-full max-w-md">
          ...
          >
            <div className="flex flex-col items-center">
              ...
              <motion.h1 variants={item} className="self-start text-2xl sm:text-[1.7rem] font-bold tracking-tight text-gray-900 mb-1">
                Create your account
              </motion.h1>
              <motion.p variants={item} className="self-start text-sm text-gray-500 mb-7">
                Join Alákòwé — it only takes a minute
              </motion.p>
              ...
              <motion.div variants={item} className="flex flex-col w-full mb-5 gap-y-3">
                <motion.button
                  ...
                  className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 border border-gray-200 hover:border-gray-300 text-gray-700 font-semibold py-3.5 rounded-full text-sm shadow-sm hover:shadow-md transition-all"
                >
              ...
              <motion.form variants={item} onSubmit={handleSubmit} className="w-full flex flex-col gap-3.5">
                <input
                  type="text"
                  required
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-5 top-1/2 -translate-y-1/2 text-xs font-semibold text-gray-500 hover:text-gray-900 transition-colors"
                  >
                    {showPwd ? 'Hide' : 'Show'}
                  </button>
                </div>
                ...
                <AnimatePresence>
                  {password.length > 0 && (
                    ...
                    >
                      ...
                      <p className="mt-1.5 text-[11px] text-gray-500">{strengthLabel}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <label className="flex items-start gap-2.5 mt-1 select-none cursor-pointer">
                  <span className="relative mt-0.5">
                    ...
                    />
                    <span className="h-[18px] w-[18px] rounded-md border border-gray-300 bg-white grid place-items-center transition-all peer-checked:bg-gray-900 peer-checked:border-gray-900 peer-focus-visible:ring-4 peer-focus-visible:ring-gray-900/10">
                      <svg viewBox="0 0 20 20" className={`h-3 w-3 text-white transition-opacity ${agree ? 'opacity-100' : 'opacity-0'}`} fill="none" stroke="currentColor" strokeWidth="3">
                  ...
                  <span className="text-xs text-gray-500 leading-relaxed">
                    I agree to the{' '}
                    <Link to="/terms" className="text-gray-800 underline underline-offset-2 hover:text-black">Terms of Service</Link>
                    {' '}and{' '}
                    <Link to="/privacy" className="text-gray-800 underline underline-offset-2 hover:text-black">Privacy Policy</Link>.
                  </span>
                </label>
          ...
          <motion.p variants={item} className="mt-6 text-center text-sm text-gray-600">
            ...
            <Link to="/login" className="relative font-semibold text-gray-900 group inline-block">
